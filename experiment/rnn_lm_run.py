...
def data(code_set:IdList, batch_size:int=32, epoches:int=10) -> typing.Iterator[IdList]:
    for i in range(epoches):
        code_set = sklearn.utils.shuffle(code_set)
        for t in range(0, len(code_set), batch_size):
            yield code_set[t:t+batch_size]

def train(model_parameters, train_data, val_data, epoches=15, skip_steps=5, print_skip_step=100, save_steps=1000):
    ...
    for i, codes in enumerate(data(train_data, epoches=epoches)):
        _, loss = sess.run([train_op, model.loss_op], feed_dict={X_input: train_data})
        losses.append(loss)
        if i % skip_steps == 0:
            summary = sess.run(model.summary_op, feed_dict={X_input: train_data})
...
if __name__ == '__main__':
    code_list = read_code_list(get_right_C_code, 10000)
    ...
    for param in param_generator(1):
        ...
        train(param, train_data, val_data)
