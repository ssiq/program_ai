...
def parse_xy(df, keyword_voc, char_voc):
    ...
    def create_error_list(one):
        ...
        import json
        original_code = one['ac_code']
        error_code = one['code']
        error_count = one['error_count']
        error_character_maplist = json.loads(one['error_character_maplist'])
        action_character_list = json.loads(one['action_character_list'])
        ac_code_obj = one['ac_code_obj']
        ac_code_name = one['ac_code_name']

        start_ac_pos = []
        for act in action_character_list:
            ac_pos = act['ac_pos']
            for obj in ac_code_obj:
                if ac_pos >= obj.start:
    ...
    df['ac_code_name'] = df['ac_code_obj'].map(token_to_name_list)
