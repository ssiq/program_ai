...
class CharacterLanguageModel(object):
    ...
    def loss_op(self):
        ...
        print(util.get_shape(logits))
        print(util.get_shape(labels))
        loss = tf.nn.softmax_cross_entropy_with_logits(logits=logits,
                                                       labels=labels)
        loss = tf.reduce_mean(loss)
        return loss

    @util.define_scope(scope="sample")
    def sample_op(self):
    ...
    def logit_op(self):
        ...
        init_state = tf.placeholder_with_default(default_init_state, shape=(None, util.get_shape(default_init_state)[1]))
        sequence_length = util.length(self.one_hot_X)
    ...
    @util.define_scope(scope='embedding')
    ...
    @util.define_scope(scope='perplexity')
    def perplexity_op(self):
        return util.perplexity(self.logit_op[0], self.one_hot_X)
