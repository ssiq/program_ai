...
from common import tf_util, code_util, rnn_util, util, rnn_cell
...
class TokenLevelMultiRnnModelGraph(tf_util.BaseModel):
    ...
    def _cal_next(self, _memory, _memory_length, _position_embedding, _position_length, inputs):
        ...
        (m_forward, m_backward), last_hidden_state = rnn_util.bi_rnn(lambda: rnn_cell.GatedAttentionWrapper(
            self._multi_rnn_cell(self.hidden_state_size, self.rnn_layer_number),
            tf.expand_dims(position_softmax, axis=2) * _position_embedding,
            self.position_length_op,
            self.hidden_state_size,
        )
                                                     , _memory, _memory_length,)
        replace_ouput = rnn_util.soft_attention_reduce_sum([m_forward, m_backward], replace_input, self.hidden_state_size,
                                                           _memory_length)[0]
        ...
        is_copy_logit = _multi_layer(replace_ouput, 1, "copy_weight", 3)
        # is_copy_logit = tf_util.weight_multiply("copy_weight", replace_ouput, 1)[:, 0]
        # key_word_logit
        replace_ouput_wared_copy_logit = tf.concat((replace_ouput, is_copy_logit), axis=-1)
        key_word_logit = _multi_layer(replace_ouput_wared_copy_logit, self.keyword_number, "key_word_logit_weight", 3)
        ...
        with tf.variable_scope("copy_word_logit"):
            copy_word_state = _multi_layer(replace_ouput_wared_copy_logit, self.hidden_state_size, "copy_word_weight", 3)
            ...
            copy_word_softmax = tf_util.variable_length_softmax(copy_word_logit, _memory_length)
            copy_word_o = tf.add_n(rnn_util.reduce_sum_with_attention_softmax([m_forward, m_backward], copy_word_softmax))
        is_copy_logit = is_copy_logit[:, 0]
