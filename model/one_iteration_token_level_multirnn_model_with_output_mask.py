...
class TokenLevelMultiRnnModelGraph(tf_util.BaseModel):
    ...
    def softmax_op(self):
        copy_word_logit, is_continue_logit, is_copy_logit, key_word_logit, position_logit = self.logit_op
    ...
    def loss_op(self):
        copy_word_logit, is_continue_logit, is_copy_logit, key_word_logit, position_logit = self.logit_op
        loss = tf.reduce_mean(
                        tf.nn.sigmoid_cross_entropy_with_logits(logits=is_continue_logit,
                                                                labels=tf.cast(
                                                                    self.output_is_continue,
                                                                    tf.float32)))
        loss += tf.reduce_mean(
        ...
        position_loss = tf.reduce_sum(
            sparse_categorical_loss(self.output_position_label, self.softmax_op[1]))
    ...
    def predict_op(self):
        ...
        output = (tf.nn.sigmoid(is_continue_logit),
                  tf_util.variable_length_softmax(position_logit, self.position_length_op),
